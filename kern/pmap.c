...
mem_init_mp(void)
{
	...
	//	cprintf("NCPU= %d\n",NCPU);
	 ...
	 // cprintf("kstacktop_i %08x, PADDR %08x\n",kstacktop_i-KSTKSIZE,PADDR(percpu_kstacks[i]));
	}
/*	cprintf("i= %d\n",i);
	kstacktop_i = KSTACKTOP-i*(KSTKSIZE+KSTKGAP);
        boot_map_region(kern_pgdir,kstacktop_i-KSTKSIZE,KSTKSIZE,PADDR(percpu_kstacks[i]),PTE_W);	   
...
//
// Map [va, va+size) of virtual address space to physical [pa, pa+size)// in the page table rooted at pgdir.  Size is a multiple of PGSIZE, and
